...
const QuoteList = (props) => {
  ...
  const queryParams = new URLSearchParams(location.search); //location.search has the query params
  const isSortAscending = queryParams.get("sort") === "asc";

  const sortedQuotes = sortQuotes(props.quotes, isSortAscending);

  const changeSortingHandler = () => {
    //alternate approach to show that object can also be passed in push method of useHistory hook
  /*   history.push({
